...
if not torch.cuda.is_available():
    print("❌ GPU not available! Run on Google Colab or GPU machine.")
    exit(1)
...
os.makedirs('graphs_gpu', exist_ok=True)
os.makedirs('best_gpu_models', exist_ok=True)
...
def plot_history(history, model_type, batch_size, optimizer, lr, compute='GPU'):
    ...
    save_path = os.path.join('graphs_gpu', filename)
...
for config in configs:
    for model_type in models:
        ...
        plot_history(result['history'], model_type, config['batch_size'], 
                    config['optimizer'], config['lr'], compute='GPU')
...
for model_type in ['resnet18', 'resnet50']:
    ...
    filename = (f"best_{model_type}_GPU_bs{config['batch_size']}_"
...
csv_path = 'results_gpu/fashionmnist_gpu_results.csv'
